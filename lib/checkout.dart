  ...
  Future<int> recordcheckout() async {
    ...
    for (int i = 0; i < guestinfoout.length; i++) {
      ...
      if (guestinfoout[i]["uid"] == uid) {
        ...
        foundout = i;
        checkoutdetail.add(snapshot["checkout"][i]);
...
Widget buildImageInteractionCard(BuildContext context) {
  String name = checkoutdetail[0]["name"];
  String roomnos = "";
  for (int i = 0; i < checkoutdetail[0]["roomno"].length; i++) {
    String c = checkoutdetail[0]["roomno"][i].toString();
    if (i == 0) {
      roomnos = roomnos + '$c';
    } else {
      roomnos = roomnos + ', $c';
...
void checkoutdone() async {
  ...
  DocumentReference _stock = _stocks.doc("Premier");
  DocumentSnapshot snapshot = await _stock.get();
  List<dynamic> roomrestore = checkoutdetail[0]["roomno"];
